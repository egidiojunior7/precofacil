...
const PricingCalculator: React.FC<PricingCalculatorProps> = ({ session }) => {
    ...
    const { totalHourlyFixedCost, totalMonthlyFixedCost } = useMemo(() => {
        ...
        const totalHoursInMonth = (laborData.workingDays * laborData.workingHours) > 0 ? (laborData.workingDays * laborData.workingHours) : (22*8);
    ...
    if (loading) return <div className="flex items-center justify-center min-h-screen"><p className="text-lg text-slate-600">Carregando seus dados...</p></div>;

    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-2xl font-bold text-slate-900">Painel de Precificação</h1>
                <p className="mt-1 text-sm text-slate-600">Calcule o preço de venda dos seus produtos de forma precisa.</p>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 items-start">
                ...
                <div className="space-y-8 lg:col-span-2 xl:col-span-1">
                    <Card title="Passo 5: Dados do Produto" icon={<TagIcon />}><div className="mb-2"><label className="block text-sm font-medium text-slate-700 mb-1">Nome do Produto</label><input type="text" value={productData.name} onChange={(e) => handleProductChange('name', e.target.value)} className="block w-full text-sm px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ex: Cartão de Visita" /></div><NumberInput label="Tempo Gasto (Horas)" value={productData.timeSpent} onChange={(v) => handleProductChange('timeSpent', v)} step={0.1} /><NumberInput label="Quantidade de Impressões" value={productData.printQuantity} onChange={(v) => handleProductChange('printQuantity', v)} /><NumberInput label="Margem de Lucro Desejada (%)" value={productData.desiredProfitMargin} onChange={(v) => handleProductChange('desiredProfitMargin', v)} suffix="%" /></Card>
                    <div className="sticky top-8"><Card title="Resultado da Precificação" icon={<PriceIcon className="h-6 w-6 text-white"/>} className="bg-indigo-600 text-white shadow-lg shadow-indigo-500/20"><h2 className="text-lg font-bold text-center text-indigo-200 truncate">{productData.name || "Seu Produto"}</h2><div className="my-6 text-center"><p className="text-sm font-medium text-indigo-200">Preço de Venda Sugerido</p><p className="text-4xl font-extrabold tracking-tight text-white py-1">{formatCurrency(suggestedPrice)}</p><p className="text-sm text-indigo-300">Sugestão arredondada: {formatCurrency(roundedSuggestedPrice)}</p></div><div className="space-y-2"><label htmlFor="manualPrice" className="block text-sm font-medium text-indigo-200">Seu Preço de Venda (Opcional)</label><div className="relative"><span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-indigo-300">R$</span><input id="manualPrice" type="number" value={manualSellingPrice} onChange={(e) => setManualSellingPrice(e.target.value)} min={0} placeholder={suggestedPrice.toFixed(2)} className="w-full bg-indigo-700 border border-indigo-500 rounded-md py-2 pl-10 pr-3 text-white placeholder-indigo-300 focus:outline-none focus:ring-2 focus:ring-white" /></div></div><div className="space-y-3 text-sm border-t border-indigo-500 pt-6 mt-6"><div className="flex justify-between items-center text-indigo-200"><span>Custo Total de Produção</span><span className="font-semibold">{formatCurrency(totalBaseCost)}</span></div><div className="flex justify-between items-center text-white font-bold"><span>Lucro ({finalProfitMargin.toFixed(1)}%)</span><span>{formatCurrency(finalProfitAmount)}</span></div>{Number(manualSellingPrice) > 0 && Number(manualSellingPrice) < suggestedPrice && (<p className="text-xs text-yellow-300 mt-1 text-center">Atenção: Seu preço está abaixo do sugerido.</p>)}</div></Card></div>
                </div>
            </div>
        </div>
    );
