...
const PricingCalculator: React.FC<PricingCalculatorProps> = ({ session }) => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (laborError && laborError.code === 'PGRST116') {
                    const { data: newLabor, error: insertError } = await supabase.from('labor_data').insert({ user_id: user.id, desiredSalary: 1500, workingDays: 20, workingHours: 8 }).select().single();
                ...
                let { data: product, error: productError } = await supabase.from('product_data').select('*').eq('user_id', user.id).single();
                 if (productError && productError.code === 'PGRST116') {
                    const { data: newProduct, error: insertError } = await supabase.from('product_data').insert({ user_id: user.id, name: 'Meu Primeiro Produto', timeSpent: 1, printQuantity: 100, desiredProfitMargin: 50 }).select().single();
                    if (insertError) throw insertError;
                    product = newProduct;
                ...
                const { data: inks, error: inkError } = await supabase.from('ink_costs').select('id, name, cartridge_price, cartridge_yield').eq('user_id', user.id).order('created_at');
                if (inkError) throw inkError;
                if (inks) setInkCosts(inks.map(i => ({ id: i.id, name: i.name, cartridgePrice: i.cartridge_price, cartridgeYield: i.cartridge_yield })));
            } catch (error) {
    ...
    const handleAddFixedCost = async () => {
        const { data } = await supabase.from('fixed_costs').insert({ name: 'Novo Custo', monthlyCost: 0, user_id: session.user.id }).select().single();
    ...
    const handleAddVariableCost = async () => {
        const { data } = await supabase.from('variable_costs').insert({ name: 'Novo Material', itemCost: 0, yield: 1, quantity: 0, unitName: 'unidades', user_id: session.user.id }).select().single();
    ...
    const handleInkCostChange = async (id: string, field: keyof Omit<InkCost, 'id'>, value: string | number) => {
        ...
        const dbField = field === 'cartridgePrice' ? 'cartridge_price' : field === 'cartridgeYield' ? 'cartridge_yield' : field;
        await supabase.from('ink_costs').update({ [dbField]: value }).eq('id', id);
    };
    ...
    const handleAddInkCost = async () => {
        ...
        if (data) setInkCosts(costs => [...costs, { id: data.id, name: data.name, cartridgePrice: data.cartridge_price, cartridgeYield: data.cartridge_yield }]);
    };
    ...
    const { totalHourlyFixedCost, totalMonthlyFixedCost } = useMemo(() => {
        ...
        return { totalHourlyFixedCost: monthlyTotal / 720, totalMonthlyFixedCost: monthlyTotal };
    }, [fixedCosts]);
